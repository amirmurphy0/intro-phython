...
def count_limbs(animal, quantity):
    if animal =="cow":
        return 4* quantity
    elif animal == "chicken":
        return 2* quantity
...
#Define a dictionary to store information about the team members
team_members = {john :["What is john's favorite color?","What is john's favorite food?"],
'sarah':["what is sarah's favorite hobby?,'what is sarah's favorite movie?"]}
#initialize variables
total_questions = 0
correct_answers = 0
incorrect_answers = 0
#Ask the user questions about each team member
for member in team_members:
    questions = team_members[member]
    for question in questions:
        answer = input ("what is the answer to the questions" +question )